...
def update_test_losses(config, a1, a2):
    filename = f"results/new/results_b{config['batch_size']}{config['demo']}/corr_ratio_{config['corr_ratio']}/clin_frac_{config['client_fractions']}/test_losses.json"
    ...
    data[str(config['miss_ratio'])][config['fed_name']] = (a1, a2)
...
def update_downstream_results(config, acc_mean, acc_std, f1_mean, f1_std):
    filename = f"results/new/results_b{config['batch_size']}{config['demo']}/corr_ratio_{config['corr_ratio']}/clin_frac_{config['client_fractions']}/test_results.json"
    ...
    if config['imputation']:
        data[config['downstream_column']][config['fed_name']] = {
            'acc_mean': acc_mean,
            'acc_std': acc_std,
            'f1_mean': f1_mean,
            'f1_std': f1_std
        ...
        data[config['downstream_column']][config['na_impute']] = {
            'acc_mean': acc_mean,
            'acc_std': acc_std,
            'f1_mean': f1_mean,
            'f1_std': f1_std
...
def update_downstream_clf(config, fpr, tpr, auc, auc_std, pr_aucs, pr_auc, pr_auc_std, precision, recall):
    filename = f"results/new/results_b{config['batch_size']}{config['demo']}/corr_ratio_{config['corr_ratio']}/clin_frac_{config['client_fractions']}/downstream_clf.pkl"
    try:
    ...
    if config['imputation']:
        data[config['downstream_column']][config['fed_name']] = {'fpr': fpr,
                                                                 'tpr': tpr,
                                                                 'auc': auc,
                                                                 'auc_std': auc_std,
                                                                 'pr_aucs': pr_aucs,
        ...
        data[config['downstream_column']][config['na_impute']] = {'fpr': fpr,
                                                                  'tpr': tpr,
                                                                  'auc': auc,
                                                                  'auc_std': auc_std,
                                                                  'pr_aucs': pr_aucs,
