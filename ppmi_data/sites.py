...
def get_sites(ppmi, replication, miss_ratio, corr_ratio):
    train_sites = [26, 36, 31, 14, 72, 23, 19, 21, 70, 28, 67, 29, 25, 37, 71, 10, 24, 61, 32, 63, 35, 74, 13, 34, 22, 42, 64]
    ...
    def get_loader(dataset, ratio, modality):
        return [SampleDataset(*process(ds.to_numpy(), ratio, modality, replication)) for ds in dataset]
...
def remove_modalities(dataset, replication, cor_r):
    for row in dataset:
        # print(row)
        # print(get_modalities(cor_r, replication))
        for i, mask in enumerate(get_modalities(cor_r, replication)):
            if mask:
                row[i] = np.nan
    return dataset
...
def get_modalities(corr_r, replication):
    modalities = []
    if replication:
        ...
        # SITE,PATNO,visit_name,age_at_baseline,SEX,HISPLAT,race,COHORT,ess,updrs1PQ_score,updrs1_score,updrs2_score,NHY,updrs3_score,updrs4_score,moca,MSEADLG,rem,upsit
        modalities.append([0, 0, 0, 0, 0])  # visit_name	age_at_baseline	SEX	HISPLAT	race
        modalities.append([1])  # ess
        modalities.append([2, 2])  # updrs1PQ_score,updrs1_score
        modalities.append([3])  # updrs2_score
        modalities.append([4]) # NHY
        modalities.append([5]) # updrs3_score
        modalities.append([6])  # updrs4_score
        modalities.append([7])  # moca
        modalities.append([8]) # MSEADLG
        modalities.append([9])  # rem
        modalities.append([10])  # upsit
